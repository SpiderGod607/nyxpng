...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk, String> {
        if let Some(index) = self
            .chunks
            .iter()
            .position(|c| c.chunk_type().as_string() == chunk_type)
        {
            return Ok(self.chunks.remove(index));
        }

        return Err("chuck with type not found".to_string());
    }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        return self
            .chunks()
            .iter()
            .find(|&c| String::from_utf8_lossy(&c.chunk_type().bytes()) == chunk_type);
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        loop {
            ...
            //            let chunk_type_str = chunk_type.as_string();
            let data = read_chunk_data(&mut reader, (length) as usize)
...
fn read_u32(reader: &mut impl Read) -> Result<u32, String> {
    ...
    return Ok(u32::from_be_bytes(buff));
}

fn read_chunk_type(reader: &mut impl Read) -> Result<ChunkType, String> {
    let mut buff = [0u8; 4];
    reader.read_exact(&mut buff).map_err(|e| e.to_string())?;
    return ChunkType::try_from(buff).map_err(|e| e.to_string());
}

fn read_chunk_data(reader: &mut impl Read, length: usize) -> Result<Vec<u8>, String> {
...
impl fmt::Display for Png {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln!(f, "PNG File: {} chunks", self.chunks.len())?;
...
mod tests {
    ...
    fn test_valid_from_bytes() {
        ...
        let bytes: Vec<u8> = PNG_SIGNATURE
            .iter()
    ...
    fn test_as_bytes() {
        ...
        let aa = png.chunks.first().unwrap();

        print!(" asdf {}", aa.length());
